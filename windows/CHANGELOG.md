...
- 根目录加入 `Windows用户点这里.cmd`，自动优先选择 PowerShell 7，未安装时回退到系统 PowerShell 5.1，并在安装后直接启动皮肤。
- 安装前询问欢迎昵称并持久保存；PowerShell 版本继续自动检测，不要求用户手动选择。
- 加入与 macOS 共用的小龙女背景、固定欢迎语、原生四胶囊、Codex logo 与四条薄荷色连接线定稿。

### 修复

- 状态 JSON 读取按 PowerShell 版本选择参数，PowerShell 5.1 与 7 现在通过同一套回归测试。
