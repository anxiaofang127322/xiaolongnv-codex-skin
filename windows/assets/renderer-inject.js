((cssText, artDataUrl, logoDataUrl, nickname) => {
  ...
  const updateConnectorMap = (chrome, home) => {
    ...
    if (!orbit || !svg || !disc || paths.length !== 4 || dots.length !== 8 || buttons.length !== 4) return;
    ...
    const lastCenterY = buttonBoxes[3].top + buttonBoxes[3].height / 2;
  ...
  const ensure = () => {
    ...
    if (chrome.querySelector(".dream-connector-map")?.dataset.layout !== "dynamic-v1") {
      chrome.innerHTML = `
        <div class="dream-orbit-logo" aria-hidden="true">
          <svg class="dream-connector-map" data-layout="dynamic-v1" viewBox="0 0 170 210" aria-hidden="true">
