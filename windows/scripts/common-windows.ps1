...
function Start-DreamSkinCodexApp {
  param(
    [Parameter(Mandatory = $true)][object]$Codex,
    [string[]]$ArgumentList = @()
  )

  if (-not ('CodexDreamSkin.AppxLauncher' -as [type])) {
...
'@
  ...
  return [CodexDreamSkin.AppxLauncher]::Start($appUserModelId, $arguments)
}
...
function Get-DreamSkinNodeRuntime {
  ...
  $nodePath = if ($command) { $command.Source } else { $null }
  if (-not $nodePath) {
  ...
  if (-not $nodePath) { throw "Node.js $MinimumMajor or newer is required and was not found in PATH or the Codex runtime cache." }
  ...
  $runtimePath = "$(& $nodePath -p 'process.execPath' 2>$null)".Trim()
...
function Read-DreamSkinState {
  ...
  try {
    $json = Read-DreamSkinUtf8File -Path $Path
    $state = if ($PSVersionTable.PSVersion.Major -ge 7) {
