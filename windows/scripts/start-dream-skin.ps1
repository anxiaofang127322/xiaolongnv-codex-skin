...
try {
  ...
  try {
    if ($null -eq (Get-DreamSkinVerifiedCdpIdentity -Port $Port -Codex $codex)) {
      ...
      Start-DreamSkinCodexApp -Codex $codex -ArgumentList $arguments | Out-Null
    ...
    if (($closedExistingCodex -or $launchedWithCdp) -and
      ...
      try { Start-DreamSkinCodexApp -Codex $codex | Out-Null } catch {
  ...
  try {
    ...
    if ($launchedWithCdp) {
      try {
        ...
        Start-DreamSkinCodexApp -Codex $codex | Out-Null
      } catch {
