...
```text
请安装这个 Codex 皮肤：https://github.com/anxiaofang127322/xiaolongnv-codex-skin 。请下载完整仓库并阅读根目录 AGENTS.md。安装前先问我：“默认名称为【李嘉图】，请告诉我您要修改成什么昵称？” Windows 请自动检测 PowerShell 7 或 5.1，不要让我猜版本。按当前系统完成测试、安装和首页验证，重启前先告诉我，并说明昵称、使用方法、注意事项和恢复方式。
```
...
3. 安装器会询问欢迎昵称，然后自动检测 PowerShell：优先使用 7，没有时回退到系统 PowerShell 5.1，不需要手动选择。
...
- 默认欢迎语为“欢迎回来，李嘉图”，安装时可修改昵称并持久保存。Mac 版支持使用 `macos/Customize Codex Dream Skin.command` 更换背景。
