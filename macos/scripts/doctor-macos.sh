...
for required in \
  "$PROJECT_ROOT/assets/xiaolongnv-skin.css" \
  "$PROJECT_ROOT/assets/xiaolongnv-renderer-inject.js" \
  "$PROJECT_ROOT/assets/dream-reference.png" \
  "$PROJECT_ROOT/assets/codex-logo.png" \
...
if [ -f "$STATE_PATH" ] && verified_cdp_endpoint "$PORT"; then
  ...
  [ "$REQUIRE_LIVE" = "false" ] || VERIFY_MODE="--verify"
  if "$NODE" "$INJECTOR" "$VERIFY_MODE" --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 12000 >/dev/null 2>&1; then
    LIVE="true"
  fi
fi
