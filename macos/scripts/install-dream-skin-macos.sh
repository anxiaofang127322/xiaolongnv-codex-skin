...
remove_managed_desktop_launchers() {
  local target
  for target in \
    ...
    if [ -f "$target" ] && /usr/bin/grep -q '^# CodexDreamSkinStudio launcher$' "$target" 2>/dev/null; then
      /bin/rm -f "$target"
    fi
  done
}
...
printf 'No Desktop files were created. Use the commands in the macOS package folder when needed.\n'
