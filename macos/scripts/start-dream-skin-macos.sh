...
set +e
"$NODE" "$INJECTOR" --verify-runtime --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 20000 >/tmp/dream-skin-verify.$$.json 2>/dev/null
verify_code=$?
set -e
if [ "$verify_code" -ne 0 ]; then
  ...
  set +e
  "$NODE" "$INJECTOR" --verify-runtime --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 12000 >/tmp/dream-skin-verify.$$.json 2>/dev/null
  verify_code=$?
  set -e
fi
if [ "$verify_code" -ne 0 ]; then
